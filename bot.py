...
async def start(bot, cmd):
	...
	if usr_cmd == "/start":
		...
		try:
			...
			await send_stored_file.reply_text(f"**Here is Sharable Link of this file:** https://telegram.dog/{BOT_USERNAME}?start=jerin_{file_id}\n\n__To Retrive the Stored File, just open the link!__", disable_web_page_preview=True, quote=True)
...
async def main(bot, message):
	if message.chat.type == "private":
		...
		try:
			...
			share_link = f"https://telegram.dog/{BOT_USERNAME}?start=jerin_{file_er_id}"
			await editable.edit(
